import java.util.Arrays;

////////remove duplicate and print the array  //1

//import java.util.Arrays;
//
//public class Solution {
//
//	
//	
//	public static void main(String[] args) 
//	{
//		
//		
//		int arr[]= {1,1,1,5,9,8,8,66,66,4,4,4};
//		
//		
//
//		for(int i=0;i<arr.length;i++)
//		{
//			System.out.print(arr[i]+" ");
//
//		}
//		System.out.println();
//		Arrays.sort(arr);
//		int withoutduplicate[]=new int[arr.length];
//		
//		int j=0;
//		for(int i=0;i<arr.length-1;i++)
//		{
//			if(arr[i]!=arr[i+1])
//			{
//				withoutduplicate[j]=arr[i];
//				j++;
//			}
//		}
//		withoutduplicate[j]=arr[arr.length-1];
//		
//		for(int i=0;i<=j;i++)
//		{
//			System.out.print(withoutduplicate[i]+" ");
//
//		}
//	}
//}



///////////////find maximum element in the array  //2


//public class Solution
//{
//	public static void main(String[] args) 
//	{
//		int [] arr= {1,2,5,99,88,5,6,77,5};
//		int maximum=arr[0];
//		for(int i=0;i<arr.length-1;i++)
//		{
//			if(maximum<arr[i])
//			{
//				maximum=arr[i++];
//			}
//		}
//		
//		System.out.println(maximum);
//	}
//}




//////////////reverse the array  //3

//public class Solution{
//	public static void main(String[] args) {
//		int [] arr= {1,5,6,5,5,55,8,9,7,6,4};
//		
//		int [] arr1=new int[arr.length];
//		
//		int j=0;
//		for(int i=arr.length-1;i>=0;i--)
//		{
//			arr1[j]=arr[i];
//			j++;
//		}
//		
//		for(int i=0;i<arr1.length;i++)
//		{
//			System.out.print(arr1[i]+" ");
//		}
//	}
//}
//




//array sort  //4

//public class Solution{
//	public static void main(String[] args) {
//		int [] arr= {1,2,5,88,0,1,4,99,77,4};
//		int temp=0;
//		for(int i=0;i<arr.length;i++)
//		{
//			for(int j=0;j<arr.length;j++)
//			{
//				if(arr[i]<arr[j])
//				{
//					temp=arr[i];
//					arr[i]=arr[j];
//					arr[j]=temp;
//				}
//			}
//		}
//		
//		for(int i=0;i<arr.length;i++)
//		{
//			System.out.print(arr[i]+" ");
//		}
//		
//		
//	}
//}
//
//
//



//palindrone  ///////////////////5


//
//public class Solution{
//	public static void main(String[] args) {
//		int num=-121;
//		String ch=String.valueOf(num);
//		char [] ha=ch.toCharArray();
//		
//		int left=0;
//	int	right=ha.length-1;
//	
//	for(int i=0;i<ha.length;i++)
//	{
//		if(ch.charAt(left)!=ch.charAt(right))
//		{
//			System.out.println("no");
//			return;
//		}
//		left++;
//		right--;
//	}
//	System.out.println("yes");
//		
//	}
//}
//



///////occrance //6
//
//public class Solution{
//	public static void main(String[] args) {
//		int [] arr= {1,55,8,99,5,55,7,9,8,6,5};
//		int count=0;
//		for(int i=0;i<arr.length;i++)
//		{
//			for(int j=0;j<arr.length;j++)
//			{
//				if(arr[i]==arr[j])
//				{
//					count++;
//				}
//			}
//			System.out.println("count of "+ arr[i]+ " "+count);
//			count=0;
//		}
//		
//		
//		
//	}
//}




//occrance  //7
public class Solution{
	public static void main(String[] args) {
		int [] arr= {1,55,8,99,5,55,7,9,8,6,5,2,2,2};
		Arrays.sort(arr);
		int count=1;
		for(int i=0;i<arr.length-1;i++)
		{
			if(arr[i]==arr[i+1])
			{
				count++;
				continue;

			}
			System.out.println("count of "+ arr[i]+ " "+count);

			count=1;
		}
		
		
		
	}
}




















